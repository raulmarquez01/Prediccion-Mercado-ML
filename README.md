...
## Resultados en una línea (versión de 22 tickers, línea base — clasificación a 1 día)
...
## Resultados multi-horizonte (universo ampliado de 60 tickers — regresión)

El hallazgo principal de la versión ampliada: **tanto el R² como el acierto
direccional crecen de forma monótona y consistente con el plazo de inversión**:

| Horizonte | R² | Acierto direccional |
|---|---|---|
| 1 semana (5 días) | 0.0004 | 53.7% |
| 1 mes (21 días) | −0.0005 | 55.2% |
| 3 meses (63 días) | 0.0225 | 59.1% |
| 6 meses (126 días) | **0.0321** | **63.6%** |

Además: las acciones **Growth predicen consistentemente mejor que las Value** en
los 4 horizontes (brecha de 1.4 a 7.7 puntos de acierto, creciente con el plazo);
**Financial Services y Utilities** son los sectores más predecibles a largo plazo;
y la **volatilidad reciente (ATR)** es la variable más importante en los 4
horizontes de rendimiento — a diferencia del modelo de dirección a 1 día, donde
dominaban el sector y el momentum de corto plazo.

**Limitación declarada:** a horizontes largos, los rendimientos de fechas
consecutivas se superponen (no son observaciones independientes), lo que optimiza
las métricas de test frente a un escenario con datos verdaderamente independientes.
...
├── 03_multihorizonte_fix.py      # Fase B: horizonte 1 semana (versión con debugging)
├── 04_horizontes_restantes.py    # Fase B: horizontes 1 mes / 3 meses / 6 meses
├── fase_e_insights.py            # Fase E: análisis por ticker/sector/estilo/variable
...
├── app.py                        # Fase 3: app web — predicción individual + cartera
├── modelo_final.joblib           # Modelo de clasificación (dirección a 1 día)
├── modelo_horizonte_5d.joblib    # Modelos de regresión multi-horizonte (Fase B)
├── modelo_horizonte_21d.joblib
├── modelo_horizonte_63d.joblib
├── modelo_horizonte_126d.joblib
├── resumen_horizontes.csv        # Métricas comparativas de los 4 horizontes
├── insights_por_ticker.csv       # Fase E: resultados por acción y horizonte
├── insights_por_sector.csv       # Fase E: resultados por sector y horizonte
├── insights_por_estilo.csv       # Fase E: Growth vs Value vs Index
├── insights_importancia_variables.csv
...
## Roadmap (completado tras asesoría con el tutor)
...
- [x] Fase B — Modelo multi-horizonte (1 semana / 1 mes / 3 meses / 6 meses) con
      predicción de dirección **y** rendimiento % esperado. Hallazgo principal:
      R² y acierto direccional crecen monótonamente con el plazo (0.0004→0.0321
      de R², 53.7%→63.6% de acierto direccional, de 1 semana a 6 meses).
- [x] Fase C — Construcción de cartera: selección de varias acciones, monto de
      inversión manual por acción, rendimiento nominal proyectado al horizonte
      elegido. Implementado en `app.py` (modo "Armar una cartera").
- [x] Fase E — Insights: Growth le gana a Value en los 4 horizontes (brecha de
      1.4 a 7.7 puntos de acierto direccional); Financial Services y Utilities
      son los sectores más predecibles a largo plazo; la volatilidad (ATR) es
      la variable más importante en los 4 horizontes de rendimiento.
